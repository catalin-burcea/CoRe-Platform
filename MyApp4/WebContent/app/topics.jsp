	...
	<div class="row">
		<c:choose>
			 <c:when test="${groupId!=null && user == null}">
	   			<div class="row">
	   				<div class="alert alert-danger text-center"><h2><strong>
	   				You must be logged in to view or create groups</strong></h2></div>
	   			</div>
	   		</c:when>
	   		 <c:when test="${groupId!=null && user != null && usergroup == null && admin == null}">
	   			<div class="row">
	   				<div class="alert alert-danger text-center"><h2><strong>
	   				You are not authorized to view this group</strong></h2></div>
	   			</div>
	   		</c:when>
	   		<c:otherwise>
			<div class="row">
								...
								<div class="row">
									<div class="alert alert-danger text-center">
										<h1 class="text-center">
											<strong> No topic was found!</strong>
										</h1>
									</div>
								</div>
							</c:when>
						                    ...
						                    <c:forEach items="${topics}" var="topic">
							                   <li class="list-group-item">
															<div class="row" id="topic-item">
																 <div class="col-xs-12 col-sm-12">
																	<c:if test="${admin != null || usergroup.ownership=='yes' || topic.getUser().getUsername()==user.getUsername()}">
																			<form  role="form" method="POST" action="deleteTopic">	
																				...
																				<input  type="hidden" name="groupId" value="${topic.getGroup().getId()}"> 
																			</form>
																			<div>
																				<button name="openEditTopicModal" type="submit" class="btn btn-primary glyphicon glyphicon-pencil pull-right" data-toggle="modal" data-target="#editTopicModal">
																					<input  type="hidden" name="groupId" value="${topic.getId()}">
																				</button>
																			</div>
																	</c:if>
																	<div class="row">
																		...
																		<div class="col-xs-9 col-md-9 section-box">
																			<h2>		
																				...
																				<a class="no-bottom-line"
																					href="viewTopic?topicId=${topic.getId()}${groupParameter}"><strong
																					style="">see post </strong>
																				</a>
																			</h2>
																			<c:choose>
																				<c:when test="${topic.getDescription().length() > 100}">
																    ...
																    ${topic.getDescription()}
																			...
																			<div class="row rating-desc">
																				<div class="col-md-12">
																					<p>
																						<i class="glyphicon glyphicon-user"></i> by <a href="#">${topic.getUser().getUsername()}</a>
																						| <i class="glyphicon glyphicon-calendar"></i>
																						${topic.getDate()} | <i
																							class="glyphicon glyphicon-comment"></i> <a href="#">${topic.getComments().size()}
																							Comment(s)</a> |<i
																							class="glyphicon glyphicon-comment"></i> <a href="#">${topic.getReviews().size()}
																							Review(s)</a>  <i class="glyphicon glyphicon-tag"></i>
																						Tag : <a href="#"><span class="label label-info">${topic.getTag().getTitle()}</span></a>
